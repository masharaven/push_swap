...
void	ft_ra(t_stack **a, int i)
{
	...
	while (last_list->next != NULL)
		last_list = last_list->next; //получем конечный элемент с которым нужно связать лист
	rotate_list->next = NULL; //зануляем конец элемента, который хотим вставить
	last_list->next = rotate_list; //связываем послений элемент с первым, который пришел из top
	*a = first_list;//начинаем наш нынешний лист со второго элемента
	if (i == 1)
		write(1, "ra\n", 3);
}
