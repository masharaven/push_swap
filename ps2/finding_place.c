...
void	ft_help_finding_place(t_stacks *s, t_stack *b, int *action, int *buff)
{
	...
	{
		*buff = s->a->data;
		if (b->data > s->a->data) //если первый элемент в ст B больше элемента в ст A
		{
			*action += 1;
			if (b->data < s->a->next->data)//если первый эдлемент в ст B больше элемента в ст A - завершить
				break ;
			s->a = s->a->next; //перехдим на другой эдемент в стеке
		}
		else
			break ;
	}
	if (ft_smaller_element_detection(s->a, *buff, b->data) == 1)
	{
		while (s->a)
		{
			if (s->a->data < *buff && s->a->data > b->data) //если теперешнее положение элемента в стеке А меньше прошлого и оно же больше первого в стеке B
				break ;
			*action += 1;
			s->a = s->a->next;
		}
