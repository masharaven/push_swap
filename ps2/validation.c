...
int		ft_validation(int argc, char **argv)
{
	...
	while (i < argc)
	{
		...
		{
			ft_number_availability(argv[i]); //пишет ошибку если не окей
			ft_valid_str(argv[i]); //пишет ошибку если не окей
			ret = 1;
		}
		i++;
	}
...
**		тем самым обрабатываем невалидные строки.
...
void	ft_duplicate_check(t_data *new, t_stacks *stacks)
{
	...
	if (!(dup = (int *)malloc(sizeof(int) * (new->count_element))))
		exit(1);
	dup = ft_intcpy(dup, new->a, new->count_element); // зачем-то переписываем в новый массив наши числа
